...
void Widget::initUI()
{
    ...
    {
        static_cast<mypushbutton_QFrame*>(fra->parent())->setRadius(15);//父类转换成子类，否则程序崩溃
        static_cast<mypushbutton_QFrame*>(fra->parent())->setSpeed(5);
    }
...
void Widget::BtnGroupClicked(QAbstractButton* btn)
{
    ...
    else if (BtnText == "1/x")
    {
        ...
        {
            QMessageBox::critical(this, "错误", "The denominator cannot be 0!\n分母不能为0！",QMessageBox::Ok);
        }
    }
    ...
    else if (BtnText == "ln(x)")
    {
        ...
        else
        {
            QMessageBox::critical(this, "错误",
                "The operand of a logarithmic operation must be greater than 0.!\n"
                "对数运算的被操作数必须大于0！",
                QMessageBox::Ok);
        }
    }
