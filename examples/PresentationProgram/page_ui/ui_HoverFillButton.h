#ifndef UI_HOVERFILLBUTTON_H
#define UI_HOVERFILLBUTTON_H
...
public:
    ...
    void setupUi(QWidget *parent)
    {
        ...
        for (auto &btn : parent->findChildren<HoverFillButton *>())
            btn->setFillSpeed(4); // 设置填充速度,越小越快,最小为1,不同的动画类型可以考虑使用不同的速度,以达到最佳观感


        // 配合样式表使用示例
        // 配合样式表,添加阴影,形成类似发光效果
        pushButton_1->setFillBrush(QColor(255, 124, 118));//设置填充颜色
        pushButton_1->setStyleSheet("background-color:rgba(240,152,152,50);"
                                    "border-radius:10");
        pushButton_1->refreshRadius(10); // 使用样式表设置border-radius后应调用refreshRadius,同步更新绘制圆角
        pushButton_1->addShadow(0, 0, 100, QColor(255, 150, 150)); // 添加阴影

        // 设置动画类型
        // 默认值//ui.pushButton_1->setAnimationType(HoverFillButton::AnimationType::CircularFill);//进入点圆形填充
        pushButton_2->setAnimationType(HoverFillButton::AnimationType::CrossFill);         // 两侧圆形填充
        pushButton_3->setAnimationType(HoverFillButton::AnimationType::DiagonalRectangle); // 斜着的矩形填充
        pushButton_4->setAnimationType(HoverFillButton::AnimationType::BottomCircle);      // 底部圆形填充
        pushButton_5->setAnimationType(HoverFillButton::AnimationType::LeftRectangle);     // 左侧矩形填充

        // 设置文字颜色示例
        pushButton_2->setTextColor(Qt::GlobalColor::red, Qt::GlobalColor::green); // 普通状态下的颜色,悬浮状态下的颜色

        // 设置自定义填充画刷示例
        // 绘制桃红色和淡绿色的渐变背景
        QPointF point(pushButton_2->width(),pushButton_2->height());//渐变中心点
        QRadialGradient gradient(point, pushButton_2->width()/1.3, point);//QRadialGradient(圆形渐变区域中心，渐变区域半径，聚焦点坐标)
        gradient.setColorAt(0.5, QColor(170, 255, 127));
        gradient.setColorAt(1.0, QColor(255, 150, 150));
        pushButton_2->setFillBrush(QBrush(gradient));//设置渐变画刷

        parent->setObjectName("page_HoverFillButton");
        parent->setStyleSheet("#page_HoverFillButton{background-color:rgb(125,125,125);}");
    }
};

#endif // UI_HOVERFILLBUTTON_H
