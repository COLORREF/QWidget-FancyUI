...
FImage::FImage(QImage &&other) noexcept : _qimage{std::forward<QImage &&>(other)}
{
    this->inItMat();
}

FImage::FImage(const FImage &fimage) : FImage(fimage._qimage)
{
}

FImage::FImage(FImage &&fimage) noexcept : FImage(std::move(fimage._qimage))
{
}
