...
namespace fancy
{
    LineEditStyle::LineEditStyle(QLineEdit *parent) :
        QProxyStyle(nullptr),
        _radius(5),
        _parent(parent),
        _ani(new QVariantAnimation(parent))
    {
        ...
        connect(_ani, &QVariantAnimation::valueChanged, this, &LineEditStyle::setIndicator);
    }

    void LineEditStyle::drawPrimitive(PrimitiveElement element, const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
    ...
    bool LineEditStyle::eventFilter(QObject *object, QEvent *event)
    {
        if (object == _parent)
        {
    ...
    void LineEditStyle::setIndicator(const QVariant &val)
    {
        _indicator = val.toLine();
        _parent->update();
    }

    void LineEditStyle::drawBase(QPainter *painter, const QRect &rect, const ColorRole fillRole) const
    {
