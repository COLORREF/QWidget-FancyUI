...
namespace fancy
{
    TextEditStyle::TextEditStyle(QTextEdit *parent) :
        QProxyStyle(nullptr),
        _radius(5),
        _parent(parent),
        _ani(new QVariantAnimation(parent))
    {
        ...
        connect(_ani, &QVariantAnimation::valueChanged, this, &TextEditStyle::setIndicator);
    }

    void TextEditStyle::drawPrimitive(PrimitiveElement element, const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
    ...
    bool TextEditStyle::eventFilter(QObject *object, QEvent *event)
    {
        if (object == _parent)
        {
    ...
    void TextEditStyle::setIndicator(const QVariant &val)
    {
        _indicator = val.toLine();
        _parent->update();
    }

    void TextEditStyle::drawBase(QPainter *painter, const QRect &rect, const ColorRole fillRole) const
    {
