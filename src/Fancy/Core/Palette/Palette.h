...
namespace fancy
{
    ...
    private:
        ...
        /**
         * @brief 读取并解析颜色配置文件，返回对应颜色组的 json 对象
         * @param group 颜色组
         * @return 成功返回有效的 QJsonObject，失败返回空 QJsonObject
         */
        static QJsonObject loadColorProfile(ColorGroups group);

        /**
         * @brief 从 json 字符串解析 QColor
         * @param custom 自定义颜色
         * @param raw 待解析的 json 值字符串
         * @return 解析后的颜色，解析失败返回空颜色
         */
        static QColor parseColorFromJsonStr(const QColor &custom, const QString &raw);
